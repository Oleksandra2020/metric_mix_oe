...
def train():
    ...
    for x, y in batch_iter:
        ...
        distance_oe = 1 - torch.mm(embeddings_n, oe_embeddings_n.t())
        ...
        for i in range(embeddings.shape[0]):
            ...
            # random mixed pairs
            anchor_mx = np.random.choice([0, 1], 1)
            lam = np.random.beta(args.alpha, args.alpha)

            row_oe = distance_oe[i]
            negative_inds = torch.where(y != -1)[0]
            negative_oe = oe_embeddings[torch.argmin(row_oe[negative_inds])]
            negative_oe = negative_oe[None, :].repeat(positive.shape[0], 1)

            if anchor_mx:
                anchor = lam * anchor + (1 - lam) * negative_oe
            else:
                positive = lam * positive + (1 - lam) * negative_oe
            count += positive_inds.shape[0]
...
if __name__ == "__main__":
    ...
    last_row = ["io_i1_i2_rand", args.alpha, args.beta, args.epochs,
                args.batch_size, args.margin, args.mixup]

    csv_file_name = "/home/hutorole/code/metric_oe/csv_results/dataset={}_beta={}_epochs={}_margin={}_mixup={}_id={}_io_i1_i2_rand_1.csv".format(
    ...
    for spl in range(splits):
        ...
        for rn in range(runs):
            ...
            args.save_dir = os.path.join(
                ...
                'split_{}'.format(spl),
                '{}_{}_epochs={}_bs={}_ind={}_outl_class={}_outl_num={}_margin={}_mixup={}_id={}_io_i1_i2_rand_1'.format(
