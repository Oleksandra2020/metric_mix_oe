...
wandb.init(
    ...
    group="2min_conf_epochs={}".format(args.epochs),
...
def train():
    ...
    for x, y in batch_iter:
        ...
        with torch.no_grad():
            oe_logits = net(oe_x)
            smax = (F.softmax(oe_logits, dim=1)).data.cpu().numpy()
            scores = np.max(smax, axis=1)
            min_scores_ind = torch.topk(torch.tensor(scores), k=5, largest=False).indices

        indices = np.random.choice(min_scores_ind, bs, replace=True)

        # MixOE loss
        # build mixed samples
        lam = np.random.beta(args.alpha, args.alpha)
        mixed_x = lam * x + (1 - lam) * oe_x[indices]
        ...
        oe_logits = net(oe_x[indices])
        loss += -(logits.mean(1) - torch.logsumexp(oe_logits, dim=1)).mean()
