...
wandb.init(
    ...
    group="3mixup_knn_epochs={}".format(args.epochs),
...
net = data_load.load_net(args.dataset, args.split, num_classes, manifold=True)
...
def train():
    ...
    random_neighbours = np.random.choice(neighbour_num, neighbour_inds.shape[0], replace=True)
    indices = []
    for ind, r in enumerate(random_neighbours):
        indices.append(neighbour_inds[ind, r])
    oe_custom_set = Subset(oe_set, indices)
    oe_custom_loader = DataLoader(oe_custom_set, batch_size=args.batch_size, shuffle=False,
                                    num_workers=args.prefetch, pin_memory=False)
    oe_iter = iter(oe_custom_loader)
    ...
    wandb.log({"acc1": top1.avg, "acc5": top5.avg, "id_loss": id_losses.avg,
                "mixed_loss": mixed_losses.avg, "loss": losses.avg})
...
def get_knn_faiss(k=1):
    web_vision_embeddings = []

    net.eval()
    with torch.no_grad():

        for x, y in oe_loader:
            x = x.cuda()
            logits, emb = net(x, return_embeddings=True)
            web_vision_embeddings.extend(emb.data.cpu().numpy())

        web_vision_embeddings = np.array(web_vision_embeddings)
        web_vision_embeddings = (web_vision_embeddings.T / np.linalg.norm(web_vision_embeddings, 2, -1)).T

        index = faiss.IndexFlatL2(web_vision_embeddings.shape[1])
        index.add(web_vision_embeddings)
        ...
        distances, k_closest_inds = index.search(train_embeddings, k)

    print(k_closest_inds.shape)
...
def get_knn(k=1):
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in enumerate(oe_loader):
            x = x.cuda()
            y = y.cuda()

            emb = net(x, embeddings=True).detach()
            if web_vision_embeddings == []:
                web_vision_embeddings = (emb.T / torch.linalg.norm(emb, 2, -1)).T
            else:
                web_vision_embeddings = torch.cat(
                    [web_vision_embeddings, (emb.T / torch.linalg.norm(emb, 2, -1)).T], axis=0)

            # indexes.append(index)

        indices = []
        indices_all_emb = []
        print(web_vision_embeddings.shape)
        ...
        for b, (x, y) in enumerate(train_loader):
            ...
            emb = net(x, embeddings=True)
            emb = (emb.T / torch.linalg.norm(emb, 2, -1)).T
            for j, e in enumerate(emb):
                dist = torch.norm(web_vision_embeddings - e, dim=1, p=None)
                knn = dist.topk(k, largest=True)
                ind = knn.indices[0]
                indices.append(ind.cpu())

                indices_per_emb = []
                for ind_i in knn.indices:
                    indices_per_emb.append(ind_i.cpu())

                indices_all_emb.append(indices_per_emb)

        indices_all_emb = np.array(indices_all_emb)
        print(indices_all_emb)
        print(indices_all_emb.shape)

        indices = np.array(indices)
        print(indices)
        print(indices.shape)

    return indices_all_emb
...
neighbour_num = 10
neighbour_inds = get_knn(k=neighbour_num)
