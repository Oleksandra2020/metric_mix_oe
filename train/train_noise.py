...
wandb.init(
    ...
    group="noise_epochs={}".format(args.epochs),

    name="split={}_run={}".format(args.split, args.run)
)


train_set, test_set, _ = data_load.load_data(
...
if args.save_dir is None:
    mixoe_related = 'noise_mixup'
    mixoe_related += f'_None_alpha={args.alpha:.1f}_beta={args.beta:.1f}_run={args.run}'
...
def train():
    ...
    for x, y in batch_iter:
        ...
        noise_img = (1/np.sqrt(2*np.pi) * torch.exp(-torch.randn(x.shape)**2 / 2)).cuda()
        mixed_x = lam * x + (1 - lam) * noise_img

        # construct soft labels and compute loss
        oe_y = torch.ones(bs, num_classes).cuda() / num_classes
