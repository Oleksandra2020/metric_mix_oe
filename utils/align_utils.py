# Taken from AlignMix https://github.com/shashankvkt/AlignMixup_CVPR22
...
def mixup_aligned(out, y, lam):
    ...
    feat2 = out[indices].view(out.shape[0], out.shape[1], -1)
...
def mixup_oe_aligned(out, oe_out, lam):
    # out shape = batch_size x 512 x 4 x 4 (cifar10/100)

    # indices = np.random.permutation(out.size(0))
    feat1 = out.view(out.shape[0], out.shape[1], -1)  # batch_size x 512 x 16
    # batch_size x 512 x 16
    feat2 = oe_out.view(out.shape[0], out.shape[1], -1)
